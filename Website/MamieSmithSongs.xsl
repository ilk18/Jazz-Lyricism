...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="mamie-smith-songs" as="document-node()+" select="$all-songs[descendant::sung-by eq 'Mamie Smith']"/>
    <xsl:template name="xsl:initial-template">
        <html>
            <head>
                <title>Mamie Smith</title>
            ...
            <body>
                ...
                <xsl:apply-templates select="$mamie-smith-songs"/>
            </body>
        </html>
    </xsl:template>
